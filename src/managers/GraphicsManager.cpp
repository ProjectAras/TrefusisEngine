...
{
    const SDL_Rect drawRect{(int)std::round((x - player.x + floor(player.x)) * TrefusisConfig::tileSize), (int)std::round((y - player.y + floor(player.y)) * TrefusisConfig::tileSize), drawZone.w, drawZone.h};
